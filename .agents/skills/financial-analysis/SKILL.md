...
1. Preflight: `.venv/bin/python scripts/workflow_state.py gate <case_dir> financial-analysis --as-of <YYYY-MM-DD> --json`; if `ready` is false, run or wait on `financial-data-fetch` (and `company-deep-dive`) instead of writing this file with stale or missing inputs.
...
8. Record: `.venv/bin/python scripts/workflow_state.py record <case_dir> financial-analysis`.
9. Track unresolved analysis gaps under question namespace `FIN` only via `scripts/open_questions.py upsert <case_dir> --stage financial-analysis --id FIN-<slug> ...`.
