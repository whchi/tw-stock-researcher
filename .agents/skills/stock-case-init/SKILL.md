...
2. If none exists, create `companies/<stock_id>-<slug>/` using a stable, readable slug.
3. Copy the needed template shapes into the case folder and fill only facts known from the user or verified repo data.
4. In `stock-meta.json`, keep `file_references` values either `null` or repo-relative case paths.
5. Seed `research-questions.md` with core business questions, disconfirming evidence to seek, and a claim hygiene register.
6. Seed `open-questions.md` with unresolved items that should carry across sessions.
