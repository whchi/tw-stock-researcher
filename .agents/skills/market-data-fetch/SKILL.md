...
3. Require the current TDCC result to contain current-schema `metadata.data_availability`. An unavailable ownership layer blocks ownership conclusions; it is not a negative company signal.
...
5. Require current-schema `metadata.data_availability` in `market-data.json`. FinMind permission limits are recorded as missing inputs and failure reasons; `partial` lowers confidence and `unavailable` blocks the affected market conclusion. Accumulated TDCC history remains its own sourced time series and is not a substitute for a failed current refresh.
...
- Do not infer a company or market signal from a provider permission error.
