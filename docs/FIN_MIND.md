...
Use `$FIN_MIND_TOKEN` as the API token. If unset, tell the user:
```
...
token = os.environ["FIN_MIND_TOKEN"]
params = {
...
- **Missing token**: `$FIN_MIND_TOKEN` not set — remind user to export it
